...
chat_responses = ['halo', 'yes yes yes', 'present :)']
...
#                     if name in text:
#                         response = chat_responses[random.randint(
#                             0, len(chat_responses)-1)]
...
def open_link(link):
    ...
    options.add_argument(
        "user-data-dir=C:/Users/codeph_0bia/AppData/Local/Google/Chrome/User Data/")
    ...
    for i in range(0, 40):
        ...
        if "#" in class_name and not passed_everyone:
            passed_everyone = True
        elif "#" in class_name:
            press_and_release(Key.enter)
            time.sleep(1)
            break
